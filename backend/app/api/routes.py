...
from app.api.models import QueryRequest, QueryResponse, SessionInfo, SummaryResponse, NextActionsResponse
...
async def handle_query(
    ...
    suggestions = await llm_client.get_suggestions(response_text)
...
@router.post("/next-actions/{session_id}", response_model=NextActionsResponse)
async def next_actions(
    session_id: UUID,
    llm_client: LLMClient = Depends(get_llm_client),
    session_manager=Depends(get_session_manager),
):
    conversation_history = await session_manager.get_conversation_history(str(session_id))
    if not conversation_history:
        return NextActionsResponse(next_actions=[])

    latest_response = conversation_history[-1][1] if isinstance(conversation_history[-1], (list, tuple)) else conversation_history[-1]
    suggestions = await llm_client.get_suggestions(latest_response)
    return NextActionsResponse(next_actions=suggestions)
